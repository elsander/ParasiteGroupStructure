...
imbTableConcom <- out %>% unite(tmp, Imbalance, probimbal, sigstars) %>%
    tidyr::spread(key = parcon, value = tmp) %>%
    tidyr::separate(No_Concomitant,
                    c('Par_Imbalance_c', 'Par_probimbal_c',
                      'Par_sigstars'), sep = '_') %>%
    tidyr::separate(Concomitant,
                    c('ParCon_Imbalance_c', 'ParCon_probimbal_c',
                      'ParCon_sigstars'), sep = '_') %>%
...
imbDiffDC0 <- t.test(imbTableDC0$Par_Imbalance,
                     imbTableDC0$ParCon_Imbalance,
                     alternative = "two.sided", paired = TRUE)
imbDiffDC1 <- t.test(imbTableDC1$Par_Imbalance,
                     imbTableDC1$ParCon_Imbalance,
                     alternative = "two.sided", paired = TRUE)
```
...
## from CaclulateEvenness.R
evenlist <- BuildEvennessDF('../../Results/GroupModel/')
evennessDF <- evenlist$even
